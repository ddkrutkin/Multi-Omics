...
########## Paired-End trimmed fastq quality control ##########
...
for trimmed_files in $tfastq/*paired_trimmed.fastq.gz
do
...
########## Paired-End sam to bam conversion ##########
...
########## Paired-End bam file bam_stat quality control  ##########
...
do
  ...
  bam_count_file="$(basename $bam_files _sorted.bam).txt" #Creates the name of the .txt count file 

  echo "Sample: $bam_count_file" | cat >> $counts/counting_stderr.txt #Puts the name of the counting file at the top of the text file 

  #Takes bam file as input 
  #stranded is yes because of paired-endedness
  #type=gene ensures that all features get counted (i.e tRNA's, rRNA's, etc)
  #order=pos keeps the paired reads in RAM? memory until their mate is found. If the option isn't specified, the stderr will show that there are missing read mate pairs
  #idattr=locus_tag ensures that every feature (gene) will have an identifier 
  #additional-attr=gene will return the gene name, if available
  htseq-count \
  --format=bam \
  --stranded=yes \
  --order=pos \
