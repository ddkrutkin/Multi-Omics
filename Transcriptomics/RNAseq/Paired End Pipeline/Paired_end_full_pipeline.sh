...
seq_adapter=$5 #Seqencing adapter sequence (path to .fa file)
gtf_ref=$6 #GTF reference file (path to .gtf file)
...
#Runs trimmomatic on surviving reads 
for fastq_files in $fastq_gz_dir/*.fastq.gz
do
  echo "Trimming $fastq_files with trimmomatic..."

  #create a new file name to output to from trimmomatic
  trimmed_file="$(basename $fastq_files .fastq.gz)_trimmed.fastq.gz"

  echo "Sample: $trimmed_file" | cat >> $tfastq/trimmomatic_stdout.txt
  echo "Sample: $trimmed_file" | cat >> $tfastq/trimmomatic_stderr.txt

  #run trimmomatic
  trimmomatic SE \
  $fastq_files $tfastq/$trimmed_file \
...
###########################################################################
######################Single End Bowtie2 Processing########################
#Changes directory to the bowtie-indexed genome: bowtie-build command must be executed from within the directory containing the genome index
cd $indexed
...
#Creates sam files from trimmed files 
for trimmed_files in $tfastq/*.fastq.gz
do
  echo "Converting $trimmed_files into .sam file..."

  #create a new file name for output from bowtie2
  mapped_file="$(basename $trimmed_files _trimmed.fastq.gz)_mapped.sam"

  echo "Sample: $mapped_file" | cat >> $mappedSAM/sam_mapping_stdout.txt

  bowtie2 -x $organism_name \
  $trimmed_files -S $mappedSAM/$mapped_file \
...
#Changes back to the parent directory to avoid conflicts 
cd $parent_directory
...
do
  echo "Converting $sam_files to sorted BAM file..."
...
do
  ...
  mapped_name="$(basename $bam_files)"
  
  #Creates the name to rename the files to not include _mapped_sorted
  name="$(basename $bam_files _mapped_sorted.bam).bam"

  #echo $mapped_name

  #echo $name 

  mv $sortedBAM/$mapped_name $sortedBAM/$name
...
do
  ...
  bam_count_file="$(basename $bam_files _mapped_sorted.bam).txt"
...
> $counts/New_code/sample_list.txt
> $counts/New_code/file_list.txt

for count_files in $counts/New_code/*_001.txt
do

  sample_name="$(basename $count_files .txt)"
  file_name="$(basename $count_files)"

  echo $sample_name >> $counts/New_code/sample_list.txt
  echo $file_name >> $counts/New_code/file_list.txt
